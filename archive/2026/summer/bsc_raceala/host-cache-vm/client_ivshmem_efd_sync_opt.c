...
// vpac_count must be smaller than the stream_count; the streams are equally
// split between the VPACs initially the code assigned all streams to all VPACs
// considering that one protection function takes only tens to hundreds of ns,
// the idea of modelling VPACs as individual protection algorithm fails - each
// VPAC should process entire streams
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < vpac_count; i++) {
        ...
        case 0:
            ...
            while (1) {
                uint64_t curr = stream_id;
                do {

                    right = &((sv_caches + curr)->right);
                    if (ptr[curr] != *right) {

                        SV_Cache *sv_cache = sv_caches + curr;
                        Entry *entry = &(sv_cache->entries[ptr[curr]]);
                        ...
                        ptr[curr] = (ptr[curr] + 1) % NUMBER_OF_ENTRIES;
                        ctrl->vpac_ptrs[curr] =
                            (ctrl->vpac_ptrs[curr] + 1) % NUMBER_OF_ENTRIES;
                    }
                    curr += vpac_count;
                    if (curr >= stream_count) {
                        curr = i;
                    }

                } while (curr != stream_id);
    ...
    do {
        for (uint64_t stream_id = 0; stream_id < stream_count; stream_id++) {
            ...
            if (ptr[stream_id] != *right) {
                ...
                if (ptr[stream_id] == ctrl->vpac_ptrs[stream_id]) {

                    // protection algorithms for one stream
                    // are not split across VPACs
                    int vpac_id = stream_id % vpac_count;
